...
local function _get_bitg_cli_result(exitcode, stdout, stderr)
    if exitcode ~= 0 then
...
if _info.bitgreend == 'running' then
    ...
    local _exitcode, _stdout, _stderr = _exec_bitg_cli('-datadir=data', 'getblockchaininfo')
    local _success, _output = _get_bitg_cli_result(_exitcode, _stdout, _stderr)
    ...
    local _exitcode, _stdout, _stderr = _exec_bitg_cli('-datadir=data', 'mnsync', 'status')
    local _success, _output = _get_bitg_cli_result(_exitcode, _stdout, _stderr)
